...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("/doanJava/view/ProfileFXML.fxml")));

            Scene scene = new Scene(root);

            try {
                // Dấu "/" ở đầu nghĩa là bắt đầu tìm từ thư mục "src/main/resources"
                String css = getClass().getResource("/styles/fxml.css").toExternalForm();
                scene.getStylesheets().add(css);
            } catch (Exception e) {
                System.err.println("Không tìm thấy file CSS! Kiểm tra lại đường dẫn /styles/style.css");
            }
